	...
	static class GossipEntry {
		...
		public static DataResult<MCAVillagerGossips.GossipEntry> deserialize(Dynamic<?> dynamic) {
			return DataResult.unbox(
					DataResult.instance().group(dynamic.get("Target").read(DynamicSerializableUuid.CODEC),
							dynamic.get("Type").asString().map(VillageGossipType::byKey),
							dynamic.get("Value").asNumber().map(Number::intValue))
							.apply(DataResult.instance(), MCAVillagerGossips.GossipEntry::new));
		}
		...
		public <T> Dynamic<T> serialize(DynamicOps<T> dynamicOps) {
			return new Dynamic<>(dynamicOps,
					dynamicOps.createMap(ImmutableMap.of(dynamicOps.createString("Target"),
							DynamicSerializableUuid.CODEC.encodeStart(dynamicOps, this.target)
									.result().orElseThrow(RuntimeException::new),
							dynamicOps.createString("Type"),
							dynamicOps.createString(this.type.key), dynamicOps.createString("Value"),
